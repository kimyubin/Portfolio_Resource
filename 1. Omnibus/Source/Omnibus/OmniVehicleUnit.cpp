...
AOmniVehicleUnit::AOmniVehicleUnit()
{
	...
	ForwardSensingBoxComponent->OnComponentBeginOverlap.AddDynamic(this, &AOmniVehicleUnit::AOmniVehicleUnit::BeginOverlapSensor);
	...
	SteeringDistance = 50.0;
}

void AOmniVehicleUnit::BeginPlay()
{
	Super::BeginPlay();
...
void AOmniVehicleUnit::Tick(float DeltaTime)
{
...
void AOmniVehicleUnit::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	Super::SetupPlayerInputComponent(PlayerInputComponent);
}

void AOmniVehicleUnit::DriveToDestination()
{	
	// 액터에 새로운 회전값 적용
...
FRotator AOmniVehicleUnit::GetRotationFromOmniRoadSpline()
{
...
void AOmniVehicleUnit::BeginOverlapSensor(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
...
AOmniRoad* AOmniVehicleUnit::GetNearestOmniRoad() const
{
...
USplineComponent* AOmniVehicleUnit::GetTargetLane()
{
...
AOmniRoad* AOmniVehicleUnit::GetNextRouteRoad()
{
...
void AOmniVehicleUnit::GenerateRouteRoad()
{
...
void AOmniVehicleUnit::SetDriveMaxSpeed(const double InMaxSpeed /*= 1200.0*/)
{
	if(OB_IS_VALID(PawnMovement))
		PawnMovement->MaxSpeed = InMaxSpeed;	
}

void AOmniVehicleUnit::SetDriveAcceleration(const double InAcceleration /*= 4000.0*/)
{
	if (OB_IS_VALID(PawnMovement))
		PawnMovement->Acceleration = InAcceleration;
}

void AOmniVehicleUnit::SetDriveDeceleration(const double InDeceleration /*= 12000.0*/)
{
	if (OB_IS_VALID(PawnMovement))
		PawnMovement->Deceleration = InDeceleration;
}

void AOmniVehicleUnit::MoveToTarget(const AActor* DestActor)
{
