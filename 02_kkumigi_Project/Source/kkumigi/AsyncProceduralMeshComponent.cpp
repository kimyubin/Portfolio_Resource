...
#include "AsyncProceduralMeshComponent.h"
...
// #include "AsyncProceduralMeshComponentPluginPrivate.h"
#include "DynamicMeshBuilder.h"
#include "PhysicsEngine/PhysicsSettings.h"
#include "SceneInterface.h"
#include "StaticMeshResources.h"
#include "RayTracingInstance.h"
#include UE_INLINE_GENERATED_CPP_BY_NAME(AsyncProceduralMeshComponent)
...
DECLARE_STATS_GROUP(TEXT("AsyncProceduralMesh"), STATGROUP_AsyncProceduralMesh, STATCAT_Advanced);


DECLARE_CYCLE_STAT(TEXT("Create AsyncProcMesh Proxy"), STAT_AsyncAsyncProcMesh_CreateSceneProxy, STATGROUP_AsyncProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("Create Mesh Section"), STAT_AsyncAsyncProcMesh_CreateMeshSection, STATGROUP_AsyncProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("UpdateSection GT"), STAT_AsyncAsyncProcMesh_UpdateSectionGT, STATGROUP_AsyncProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("UpdateSection RT"), STAT_AsyncAsyncProcMesh_UpdateSectionRT, STATGROUP_AsyncProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("Get AsyncProcMesh Elements"), STAT_AsyncAsyncProcMesh_GetMeshElements, STATGROUP_AsyncProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("Update Collision"), STAT_AsyncAsyncProcMesh_UpdateCollision, STATGROUP_AsyncProceduralMesh);
...
class FAsyncProcMeshProxySection
{
public:
	...
	FAsyncProcMeshProxySection(ERHIFeatureLevel::Type InFeatureLevel)
	: Material(NULL)
	, VertexFactory(InFeatureLevel, "FAsyncProcMeshProxySection")
...
static void ConvertAsyncProcMeshToDynMeshVertex(FDynamicMeshVertex& Vert, const FProcMeshVertex& ProcVert)
{
...
class FAsyncProceduralMeshSceneProxy final : public FPrimitiveSceneProxy
{
public:
	...
	FAsyncProceduralMeshSceneProxy(UAsyncProceduralMeshComponent* Component)
		: FPrimitiveSceneProxy(Component)
		, BodySetup(Component->GetBodySetup())
		, MaterialRelevance(Component->GetMaterialRelevance(GetScene().GetFeatureLevel()))
	{
		// Copy each section
		const int32 NumSections = Component->AsyncProcMeshSections.Num();
		Sections.AddZeroed(NumSections);
		for (int SectionIdx = 0; SectionIdx < NumSections; SectionIdx++)
		{
			FProcMeshSection& SrcSection = Component->AsyncProcMeshSections[SectionIdx];
			if (SrcSection.ProcIndexBuffer.Num() > 0 && SrcSection.ProcVertexBuffer.Num() > 0)
			{
				FAsyncProcMeshProxySection* NewSection = new FAsyncProcMeshProxySection(GetScene().GetFeatureLevel());
				...
				for (int VertIdx = 0; VertIdx < NumVerts; VertIdx++)
				{
					...
					ConvertAsyncProcMeshToDynMeshVertex(Vert, ProcVert);
				}
	...
	virtual ~FAsyncProceduralMeshSceneProxy()
	{
		for (FAsyncProcMeshProxySection* Section : Sections)
		{
			if (Section != nullptr)
			{
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_AsyncAsyncProcMesh_UpdateSectionRT);
		...
		if(	SectionData != nullptr) 			
		{
			...
			{
				FAsyncProcMeshProxySection* Section = Sections[SectionData->TargetSection];
				...
				for(int32 i=0; i<NumVerts; i++)
				{
					...
					ConvertAsyncProcMeshToDynMeshVertex(Vertex, ProcVert);
	...
	{
		SCOPE_CYCLE_COUNTER(STAT_AsyncAsyncProcMesh_GetMeshElements);
		...
		for (const FAsyncProcMeshProxySection* Section : Sections)
		{
	...
	virtual void GetDynamicRayTracingInstances(FRayTracingInstanceCollector& Collector) override final
	{
		...
		{
			const FAsyncProcMeshProxySection* Section = Sections[SegmentIndex];
			if (Section != nullptr && Section->bSectionVisible)
			{
...
private:
	/** Array of sections */
	TArray<FAsyncProcMeshProxySection*> Sections;
...
UAsyncProceduralMeshComponent::UAsyncProceduralMeshComponent(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	bUseComplexAsSimpleCollision = true;
}

void UAsyncProceduralMeshComponent::PostLoad()
{
	Super::PostLoad();

	if (AsyncProcMeshBodySetup && IsTemplate())
	{
		AsyncProcMeshBodySetup->SetFlags(RF_Public | RF_ArchetypeObject);
	}
}

void UAsyncProceduralMeshComponent::CreateMeshSection_LinearColor(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FVector2D>& UV1, const TArray<FVector2D>& UV2, const TArray<FVector2D>& UV3, const TArray<FLinearColor>& VertexColors, const TArray<FProcMeshTangent>& Tangents, bool bCreateCollision, bool bSRGBConversion)
{
...
void UAsyncProceduralMeshComponent::CreateMeshSection(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FVector2D>& UV1, const TArray<FVector2D>& UV2, const TArray<FVector2D>& UV3, const TArray<FColor>& VertexColors, const TArray<FProcMeshTangent>& Tangents, bool bCreateCollision)
{
	SCOPE_CYCLE_COUNTER(STAT_AsyncAsyncProcMesh_CreateMeshSection);

	// Ensure sections array is long enough
	if (SectionIndex >= AsyncProcMeshSections.Num())
	{
		AsyncProcMeshSections.SetNum(SectionIndex + 1, EAllowShrinking::No);
	}

	// Reset this section (in case it already existed)
	FProcMeshSection& NewSection = AsyncProcMeshSections[SectionIndex];
	NewSection.Reset();
...
void UAsyncProceduralMeshComponent::UpdateMeshSection(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FVector2D>& UV1, const TArray<FVector2D>& UV2, const TArray<FVector2D>& UV3, const TArray<FColor>& VertexColors, const TArray<FProcMeshTangent>& Tangents)
{
	SCOPE_CYCLE_COUNTER(STAT_AsyncAsyncProcMesh_UpdateSectionGT);

	if(SectionIndex < AsyncProcMeshSections.Num())
	{
		FProcMeshSection& Section = AsyncProcMeshSections[SectionIndex];
		const int32 NumVerts = Vertices.Num();
		...
		if (bSameVertexCount)
		{
			...
			if (Section.bEnableCollision)
			{
				...
				for (const FProcMeshSection& CollisionSection : AsyncProcMeshSections)
				{
			...
			if (SceneProxy && !IsRenderStateDirty())
			{
				...
				FAsyncProceduralMeshSceneProxy* AsyncProcMeshSceneProxy = (FAsyncProceduralMeshSceneProxy*)SceneProxy;
				ENQUEUE_RENDER_COMMAND(FProcMeshSectionUpdate)
				([AsyncProcMeshSceneProxy, SectionData](FRHICommandListImmediate& RHICmdList) { AsyncProcMeshSceneProxy->UpdateSection_RenderThread(RHICmdList, SectionData); });
			}
...
void UAsyncProceduralMeshComponent::ClearMeshSection(int32 SectionIndex)
{
	if (SectionIndex < AsyncProcMeshSections.Num())
	{
		AsyncProcMeshSections[SectionIndex].Reset();
		UpdateLocalBounds();
		UpdateCollision();
...
void UAsyncProceduralMeshComponent::ClearAllMeshSections()
{
	AsyncProcMeshSections.Empty();
	UpdateLocalBounds();
	UpdateCollision();
	MarkRenderStateDirty();
}

void UAsyncProceduralMeshComponent::SetMeshSectionVisible(int32 SectionIndex, bool bNewVisibility)
{
	if(SectionIndex < AsyncProcMeshSections.Num())
	{
		// Set game thread state
		AsyncProcMeshSections[SectionIndex].bSectionVisible = bNewVisibility;
		...
		if (SceneProxy)
		{
			...
			ENQUEUE_RENDER_COMMAND(FProcMeshSectionVisibilityUpdate)(
				[AsyncProcMeshSceneProxy, SectionIndex, bNewVisibility](FRHICommandListImmediate& RHICmdList)
				{
					AsyncProcMeshSceneProxy->SetSectionVisibility_RenderThread(SectionIndex, bNewVisibility);
				});
		}
	}
}

bool UAsyncProceduralMeshComponent::IsMeshSectionVisible(int32 SectionIndex) const
{
	return (SectionIndex < AsyncProcMeshSections.Num()) ? AsyncProcMeshSections[SectionIndex].bSectionVisible : false;
}

int32 UAsyncProceduralMeshComponent::GetNumSections() const
{
	return AsyncProcMeshSections.Num();
}

void UAsyncProceduralMeshComponent::AddCollisionConvexMesh(TArray<FVector> ConvexVerts)
{
...
void UAsyncProceduralMeshComponent::ClearCollisionConvexMeshes()
{
...
void UAsyncProceduralMeshComponent::SetCollisionConvexMeshes(const TArray< TArray<FVector> >& ConvexMeshes)
{
...
void UAsyncProceduralMeshComponent::UpdateLocalBounds()
{
	FBox LocalBox(ForceInit);

	for (const FProcMeshSection& Section : AsyncProcMeshSections)
	{
...
FPrimitiveSceneProxy* UAsyncProceduralMeshComponent::CreateSceneProxy()
{
	SCOPE_CYCLE_COUNTER(STAT_AsyncAsyncProcMesh_CreateSceneProxy);

	return new FAsyncProceduralMeshSceneProxy(this);
}

int32 UAsyncProceduralMeshComponent::GetNumMaterials() const
{
	return AsyncProcMeshSections.Num();
}


FProcMeshSection* UAsyncProceduralMeshComponent::GetAsyncProcMeshSection(int32 SectionIndex)
{
	if (SectionIndex < AsyncProcMeshSections.Num())
	{
		return &AsyncProcMeshSections[SectionIndex];
	}
	else
	{
		return nullptr;
...
void UAsyncProceduralMeshComponent::SetAsyncProcMeshSection(int32 SectionIndex, const FProcMeshSection& Section)
{
	// Ensure sections array is long enough
	if (SectionIndex >= AsyncProcMeshSections.Num())
	{
	...
	AsyncProcMeshSections[SectionIndex] = Section;
...
void UAsyncProceduralMeshComponent::SetAsyncProcMeshSection(int32 SectionIndex, FProcMeshSection&& Section)
{
	// Ensure sections array is long enough
	if (SectionIndex >= AsyncProcMeshSections.Num())
	{
	...
	AsyncProcMeshSections[SectionIndex] = MoveTemp(Section);
...
FBoxSphereBounds UAsyncProceduralMeshComponent::CalcBounds(const FTransform& LocalToWorld) const
{
...
bool UAsyncProceduralMeshComponent::GetTriMeshSizeEstimates(struct FTriMeshCollisionDataEstimates& OutTriMeshEstimates, bool bInUseAllTriData) const
{
	for (const FProcMeshSection& Section : AsyncProcMeshSections)
	{
		if (Section.bEnableCollision)
...
bool UAsyncProceduralMeshComponent::GetPhysicsTriMeshData(struct FTriMeshCollisionData* CollisionData, bool InUseAllTriData)
{
	...
	// For each section..
	for (int32 SectionIdx = 0; SectionIdx < AsyncProcMeshSections.Num(); SectionIdx++)
	{
		FProcMeshSection& Section = AsyncProcMeshSections[SectionIdx];
...
bool UAsyncProceduralMeshComponent::ContainsPhysicsTriMeshData(bool InUseAllTriData) const
{
	for (const FProcMeshSection& Section : AsyncProcMeshSections)
	{
...
UBodySetup* UAsyncProceduralMeshComponent::CreateBodySetupHelper()
{
...
void UAsyncProceduralMeshComponent::CreateAsyncProcMeshBodySetup()
{
	if (AsyncProcMeshBodySetup == nullptr)
	{
		AsyncProcMeshBodySetup = CreateBodySetupHelper();
	}
}

void UAsyncProceduralMeshComponent::UpdateCollision()
{
	SCOPE_CYCLE_COUNTER(STAT_AsyncAsyncProcMesh_UpdateCollision);
	...
	else
	{
		...
		CreateAsyncProcMeshBodySetup();
	}
	
	UBodySetup* UseBodySetup = bUseAsyncCook ? AsyncBodySetupQueue.Last() : AsyncProcMeshBodySetup;
	...
	if(bUseAsyncCook)
	{
		UseBodySetup->CreatePhysicsMeshesAsync(FOnAsyncPhysicsCookFinished::CreateUObject(this, &UAsyncProceduralMeshComponent::FinishPhysicsAsyncCook, UseBodySetup));
	}
	else
	{
...
void UAsyncProceduralMeshComponent::FinishPhysicsAsyncCook(bool bSuccess, UBodySetup* FinishedBodySetup)
{
	...
	if(AsyncBodySetupQueue.Find(FinishedBodySetup, FoundIdx))
	{
		if (bSuccess)
		{
			...
			AsyncProcMeshBodySetup = FinishedBodySetup;
			RecreatePhysicsState();
...
UBodySetup* UAsyncProceduralMeshComponent::GetBodySetup()
{
	CreateAsyncProcMeshBodySetup();
	return AsyncProcMeshBodySetup;
}

UMaterialInterface* UAsyncProceduralMeshComponent::GetMaterialFromCollisionFaceIndex(int32 FaceIndex, int32& SectionIndex) const
{
	...
	if (FaceIndex >= 0)
	{
		...
		for (int32 SectionIdx = 0; SectionIdx < AsyncProcMeshSections.Num(); SectionIdx++)
		{
			const FProcMeshSection& Section = AsyncProcMeshSections[SectionIdx];
